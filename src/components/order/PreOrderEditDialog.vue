...
import { updatePreOrderItems } from '@/lib/pre-order'
import { getProducts, getProductAddonsMap } from '@/lib/product'
import { useI18n } from '@/composables/useI18n'
import { useAlertStore } from '@/stores/useAlertStore'
import type { PreOrderWithDetails, Product } from '@/types/database'
...
const emit = defineEmits<{
  ...
  saved: [preOrder: PreOrderWithDetails]
}>()
...
const availableProducts = computed(() =>
  products.value.filter((product) => product.stock_quantity > 0 && !product.is_addons),
)
...
function resetForm() {
  ...
  notes.value = props.preOrder.notes ?? ''
  items.value = props.preOrder.pre_order_items.flatMap((item) => {
...
async function handleSave() {
  ...
  const { preOrder, error } = await updatePreOrderItems(props.preOrder.id, {
  ...
  if (!preOrder) {
    alertStore.showAlert(t('alert.error'), t('order.editFailed'), 'error')
    return
  }

  alertStore.showAlert(t('alert.success'), t('order.updated'), 'success')
...
<template>
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent class="max-h-[90vh] overflow-y-auto sm:max-w-[560px]">
      <DialogHeader>
        <DialogTitle>{{ t('order.editTitle') }}</DialogTitle>
      ...
      <div v-if="preOrder" class="space-y-4">
        ...
        <div class="rounded-xl border bg-muted/30 p-4">
          ...
          <FieldGroup>
            <Field>
              <FieldLabel for="edit-preorder-product">{{ t('common.product') }}</FieldLabel>
              <ProductSearchSelect
                id="edit-preorder-product"
            ...
            <div class="flex items-end gap-3">
              <Field class="flex-1">
                <FieldLabel for="edit-preorder-add-quantity">{{ t('common.quantity') }}</FieldLabel>
                <Input
                  id="edit-preorder-add-quantity"
        ...
        <Field>
          <FieldLabel for="edit-preorder-notes">{{ t('common.notes') }}</FieldLabel>
          <Textarea
            id="edit-preorder-notes"
            v-model="notes"
